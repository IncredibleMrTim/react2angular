...
const TestAngularOne = (0, _1.react2angular)(TestOne, ['foo', 'bar', 'baz']);
const TestAngularTwo = (0, _1.react2angular)(TestTwo, ['foo', 'bar', 'baz']);
const TestAngularThree = (0, _1.react2angular)(TestThree);
const TestAngularFour = (0, _1.react2angular)(TestFour);
const TestAngularSix = (0, _1.react2angular)(TestSix, ['foo'], ['$http', '$element', 'testSixService', 'foo']);
const TestAngularSeven = (0, _1.react2angular)(TestSeven, null, ['foo']);
const TestAngularEight = (0, _1.react2angular)(TestEight, [
    'values',
    'onComponentWillUnmount',
    'onRender',
    'onChange'
]);
(0, angular_1.module)('test', ['bcherny/ngimport'])
...
(0, angular_1.bootstrap)((0, angular_1.element)(), ['test'], { strictDi: true });
describe('react2angular', () => {
    ...
    describe('initialization', () => {
        ...
        it('should use the propTypes when present and no bindingNames were specified', () => {
            const reactAngularComponent = (0, _1.react2angular)(TestFive);
        ...
        it('should use the bindingNames when present over the propTypes', () => {
            const reactAngularComponent = (0, _1.react2angular)(TestFive, ['foo']);
            expect(reactAngularComponent.bindings).toEqual({
                foo: '<'
            });
        });
        ...
        it('should have empty bindings when parameter is not passed', () => {
            expect((0, _1.react2angular)(TestThree).bindings).toEqual({});
        });
        it('should use the injectNames for DI', () => {
            const defaultDi = (0, _1.react2angular)(TestThree).controller.slice(0, -1);
            const injectedDi = (0, _1.react2angular)(TestThree, null, ['foo', 'bar']).controller.slice(0, -1);
            expect(injectedDi).toEqual(defaultDi.concat(['foo', 'bar']));
        });
        it('should have default DI specifications if injectNames is empty', () => {
            const defaultDi = (0, _1.react2angular)(TestThree).controller.slice(0, -1);
            const injectedDi = (0, _1.react2angular)(TestThree, null, []).controller.slice(0, -1);
    ...
    describe('react classes', () => {
        it('should render', () => {
            ...
            const element = (0, angular_1.element)(`<test-angular-one foo="foo" bar="bar" baz="baz"></test-angular-one>`);
        ...
        it('should render (even if the component takes no props)', () => {
            ...
            const element = (0, angular_1.element)(`<test-angular-four></test-angular-four>`);
            $compile(element)(scope);
            ngimport_1.$rootScope.$apply();
        ...
        it('should not support children', () => {
            ...
            const element = (0, angular_1.element)(`<test-angular-one foo="foo" bar="bar" baz="baz"><span>Transcluded</span></test-angular-one>`);
            $compile(element)(scope);
            ngimport_1.$rootScope.$apply();
            expect(element.find('span').length).toBe(0);
        });
        it('should take injections, which override props', () => {
            spyOn(ngimport_1.$http, 'get').and.returnValue(
            // @ts-ignore: unknown type error
            ngimport_1.$q.resolve({ data: '$http response' }));
            const scope = Object.assign(ngimport_1.$rootScope.$new(true), {
                foo: 'FOO'
            });
            const element1 = (0, angular_1.element)(`<test-angular-six foo="foo"></test-angular-six>`);
            $compile(element1)(scope);
            const element2 = (0, angular_1.element)(`<test-angular-seven foo="foo"></test-angular-seven>`);
    ...
    describe('react stateless components', () => {
        it('should render', () => {
            ...
            const element = (0, angular_1.element)(`<test-angular-two foo="foo" bar="bar" baz="baz"></test-angular-two>`);
            $compile(element)(scope);
            ngimport_1.$rootScope.$apply();
            expect(element.find('p').length).toBe(3);
        });
        it('should render (even if the component takes no props)', () => {
            const scope = ngimport_1.$rootScope.$new(true);
            const element = (0, angular_1.element)(`<test-angular-three></test-angular-three>`);
            $compile(element)(scope);
            ngimport_1.$rootScope.$apply();
            expect(element.text()).toBe('Foo');
        });
        ...
        it('should not support children', () => {
            ...
            const element = (0, angular_1.element)(`<test-angular-two foo="foo" bar="bar" baz="baz"><span>Transcluded</span></test-angular-two>`);
            $compile(element)(scope);
            ngimport_1.$rootScope.$apply();
            expect(element.find('span').length).toBe(0);
        });
        it('should not call render after component unmount', () => {
            ...
            const element = (0, angular_1.element)(`
